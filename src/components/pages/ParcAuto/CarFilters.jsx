...
const CarFilters = ({handleChangeFilters,closeButton}) => {
  ...
  return (
    <div>
      <div
        className="filters range_slider_block"
        style={!filters.minKm ? { display: "none" } : {}}
      >
        {filters.minKm && (
          <MultiRangeSlider
            filter={filters}
            ...
            max={filters.maxKm}
            onChange={({ min, max }) =>
              handleChangeFilters({ minKm: min - 1, maxKm: max + 1 })
            }
            title={"Km"}
          />
        )}
        ...
        {filters.minPrice && (
          <MultiRangeSlider
            ...
            onChange={({ min, max }) =>
              handleChangeFilters({ minPrice: min, maxPrice: max })
            }
            title={"Prix"}
          />
        )}
        ...
        {filters.minYear && (
          <MultiRangeSlider
            ...
            onChange={({ min, max }) =>
              handleChangeFilters({ minYear: min - 1, maxYear: max + 1 })
            }
            title={"Annèe"}
          />
        )}
        ...
        <div style={{ marginTop: 50 }}>
          ...
          <div className="reset container--center--row">
            Reset{" "}
            <span
              className="reset_icon_filters reset_icon"
              onClick={() => {
                setOffer(false);
                setFilters({
                  minKm: 0,
